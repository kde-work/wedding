...
class Payment_Helper {
    ...
    const PAYMENT_CHECKOUT_TRANSACTION_ID_LEGACY = 'Transaction ID';
    const PAYMENT_CHECKOUT_SUBSCRIPTION_ID = 'bambora_online_checkout_subscription_id';
    const PAYMENT_CHECKOUT_SUBSCRIPTION_ID_LEGACY = 'Subscription ID';
    ...
    public static function get_accept_url( $order ) {
        ...
        return add_query_arg( 'key', $order->order_key, add_query_arg(
                'order', Payment_Helper::is_woocommerce_3() ? $order->get_id() : $order->get_id(),
    ...
    public static function get_bambora_online_checkout_subscription_id( $subscription ) {
        ...
        $subscription_id = Payment_Helper::is_woocommerce_3() ? $subscription->get_id() : $subscription->id;
        $bambora_subscription_id = get_post_meta( $subscription_id, Payment_Helper::PAYMENT_CHECKOUT_SUBSCRIPTION_ID, true );

        //For Legacy
        if( empty( $bambora_subscription_id ) ) {
            $parent_order_id = Payment_Helper::is_woocommerce_3() ? $subscription->get_parent_id() : $subscription->parent_id;
            $bambora_subscription_id = get_post_meta( $parent_order_id, Payment_Helper::PAYMENT_CHECKOUT_SUBSCRIPTION_ID_LEGACY, true);
            if( !empty( $bambora_subscription_id ) ) {
                //Transform Legacy to new standards
                update_post_meta( $subscription_id, Payment_Helper::PAYMENT_CHECKOUT_SUBSCRIPTION_ID, $bambora_subscription_id );
                delete_post_meta( $parent_order_id, Payment_Helper::PAYMENT_CHECKOUT_SUBSCRIPTION_ID_LEGACY );
            }
        }
    ...
    public static function get_bambora_online_checkout_transaction_id($order) {
        ...
        if( empty( $transaction_id ) ) {
            $order_id = Payment_Helper::is_woocommerce_3() ? $order->get_id() : $order->get_id();
            $transaction_id = get_post_meta( $order_id, Payment_Helper::PAYMENT_CHECKOUT_TRANSACTION_ID_LEGACY, true );
            if( !empty( $transaction_id ) ) {
                //Transform Legacy to new standards
                delete_post_meta( $order_id, Payment_Helper::PAYMENT_CHECKOUT_TRANSACTION_ID_LEGACY );
