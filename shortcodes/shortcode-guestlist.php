            ...
            <form class="gl__form" data-max-id="<?php echo count($items); ?>">
                <table class="gl__table guest-table gl__table--group gl__table--<?php //echo $group['term_id'] ?>">
                    <tr class="gl__tr gl__tr--header">
                        ...
                        <th class="gl__th gl__th--notes">Notat</th>
                        <th class="gl__th gl__th--gender gl__w--106">Kjønn</th>
                    ...
                    foreach ($items as $key => $val) {
                        $i++;
//                        print_r($val);
                        echo wb_guestlist_item_template($role_list, $gender_list, $invited_to_list, $i, $val['status'], $val['name'], $val['family'], $val['role'], $val['gender'], $val['invited-to'], $val['meal'], $val['notes'], $val['phone'], $val['email'], $val['address']);
                    }
...
function wb_guestlist_item_template($role_list, $gender_list, $invited_to_list, $id, $status = '', $name = '', $family = '', $role = '', $gender = '', $invited_to = '', $meal = '', $notes = '', $phone = '', $email = '', $address = '') {
    ...
    <tr class="gl__item gl__item--<?php echo $id; ?>" data-id="<?php echo $id; ?>">
        ...
        <td class="gl__td gl__td--invited-to"><select class="guest-table__input guest-table__select guest-table__input--invited-to" name="invited-to--<?php echo $id; ?>" title="Invited to" autocomplete="off"><?php
                            ...
                            echo $html;
                                        ?></select></td>
        <td class="gl__td gl__td--contact gl__ta--center"><div class="gl-icon <?php echo (!$phone AND !$email AND !$address)?'gl-icon--empty':''; ?> gl-icon--contact" title="Contact Information in pop up"></div><!--<div class="gl-icon gl-icon--empty gl-icon--phone"></div><div class="gl-icon gl-icon--empty gl-icon--email"></div><div class="gl-ico gl-icon--emptyn gl-icon--address"></div>-->
